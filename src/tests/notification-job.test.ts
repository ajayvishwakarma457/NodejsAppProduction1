...
import { NotificationModel, NotificationDocument } from "../modules/notifications/notification.model";
...
describe("notificationJob", () => {
  ...
  it("should process a batch and mark in-app as delivered", async () => {
    ...
    const updated = await NotificationModel.findById(id).lean() as unknown as { status: string; deliveredAt: Date | null } | null;
