...
userRouter.get('/', validateMiddleware(listUsersQuerySchema), asyncHandler(userController.list));

userRouter.get('/:id', validateMiddleware(userIdParamSchema), asyncHandler(userController.getById));

userRouter.post('/', validateMiddleware(createUserSchema), asyncHandler(userController.create));
